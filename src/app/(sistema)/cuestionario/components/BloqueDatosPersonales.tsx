...
const BloqueDatosPersonales: FC<BloqueDatosPersonalesProps> = ({datosPersonalesAlmacenados = datosPersonalesInicial}) => {
  ...
  const onDatoChange = (event:React.ChangeEvent<HTMLInputElement>) =>{
    ...
    const onDatoChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // TODO: Verificar cual es el tipo de previus
        setDatosPersonalesState(
            (previus:any) => {
                return (
                    {
  ...
  const onDatosPersonalesSubmit = async (event:React.MouseEvent<HTMLButtonElement>) =>{
    ...
    const estadosCiviles:{id:number, nombre:string}[] = [
        {id: 0, nombre: 'soltero'},
        {id: 1, nombre: 'casado'},
    ];

    // TODO: Que debe hacer aqui?
    const onDatosPersonalesSubmit = () =>{
